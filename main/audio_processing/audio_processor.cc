...
void AudioProcessor::Input(const std::vector<int16_t>& data) {
    // 检查是否超过缓冲区限制
    if (input_buffer_.size() + data.size() > MAX_INPUT_BUFFER_SIZE) {
        // 如果超过限制，移除最旧的数据
        size_t remove_size = input_buffer_.size() + data.size() - MAX_INPUT_BUFFER_SIZE;
        if (remove_size > input_buffer_.size()) {
            input_buffer_.clear();
        } else {
